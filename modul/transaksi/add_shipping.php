...
// Fungsi generate nomor Shipping otomatis
function generateShippingNo($conn) {
    $year = date('Y');
    $month = date('m');
    $day = date('d');
    $date_prefix = $year . $month . $day;
    
    $query = mysqli_query($conn, "SELECT shipping_no FROM hed_shipping WHERE shipping_no LIKE 'SHP-$date_prefix-%' ORDER BY shipping_no DESC LIMIT 1");
    $row = mysqli_fetch_assoc($query);
    
    if ($row) {
        $last_num = intval(substr($row['shipping_no'], -4));
        $next_num = $last_num + 1;
    } else {
        $next_num = 1;
    }
    
    return "SHP-" . $date_prefix . "-" . str_pad($next_num, 4, '0', STR_PAD_LEFT);
}
...
// Ambil semua data UoM
$uom_rs = mysqli_query($conn, "SELECT uom_id, unit FROM m_uom WHERE is_active='Checked' ORDER BY unit ASC");
...
$shipping_date_display = formatDateIndonesian($shipping_date);
$shipping_no = generateShippingNo($conn);
            ...
            <div class="shipping-panel">
                ...
                <div class="shipping-panel-body">
                    <div class="ff">
                        ...
                        <input type="text" name="shipping_no" value="<?= htmlspecialchars($shipping_no) ?>" readonly style="font-weight:bold; color:var(--accent-blue);">
                    </div>
                    <div class="ff">
            ...
            <div class="shipping-panel">
                ...
                <div class="shipping-panel-body">
                    <div class="ff">
                        ...
                        <input type="text" name="transporter" placeholder="Nama Transporter...">
                    </div>
                    <div class="ff">
                        <label>Driver Name</label>
                        <input type="text" name="driver_name" placeholder="Nama Supir...">
                    </div>
                    <div class="ff">
                        <label>Truck No</label>
                        <input type="text" name="truck_no" placeholder="Nomor Truk...">
                    </div>
                </div>
            </div>
            ...
            <div class="shipping-panel">
                ...
                <div class="shipping-panel-body">
                    <div class="ff">
                        ...
                        <textarea name="shipment_location" id="shipment_location" rows="2" placeholder="Alamat pengiriman..."></textarea>
                    </div>
                    <div class="ff">
                        <label>Gudang</label>
...
function getUomOptions(inventoryId) {
    ...
    if (inventoryId && inventoryUomData[inventoryId]) {
        ...
        for (var i = 0; i < uomList.length; i++) {
            ...
            html += `<option value="${escHtml(uomList[i].unit)}" ${selected}>${escHtml(uomList[i].unit)}</option>`;
        }
    }
...
function updateUomOptions(row) {
    var $row = $(row);
    var inventoryId = $row.find('.inv-id').val();
    
    // Update UoM Pack options
    var uomPackSelect = $row.find('.inv-uom-pack-select');
    var currentPackValue = uomPackSelect.val();
    uomPackSelect.html(getUomOptions(inventoryId));
    if (currentPackValue) {
        uomPackSelect.val(currentPackValue);
    }
    
    // Update UoM Detail options
    var uomDetailSelect = $row.find('.inv-uom-detail-select');
    var currentDetailValue = uomDetailSelect.val();
    uomDetailSelect.html(getUomOptions(inventoryId));
    if (currentDetailValue) {
        uomDetailSelect.val(currentDetailValue);
    }
}
...
function buildRow(data) {
    ...
    var uomPackOptions = getUomOptions(inventoryId);
    var uomDetailOptions = getUomOptions(inventoryId);
    ...
    return `<tr class="detail-row" data-idx="${idx}">
        ...
        <td>
            <select name="uom_pack_shipping[]" class="inv-uom-pack-select" style="width:100%;">
                ${uomPackOptions}
            </select>
        </td>
        <td>
            <select name="uom_detail_shipping[]" class="inv-uom-detail-select" style="width:100%;">
                ${uomDetailOptions}
            </select>
        </td>
        <td>
...
function loadInventoryFromOrder() {
    ...
    // Ambil data inventory dari order via AJAX
    $.ajax({
        ...
        success: function(response) {
            if (response.success && response.data.length > 0) {
                ...
                response.data.forEach(function(item) {
                    ...
                    // Set UoM Pack dan UoM Detail dengan nilai default dari order
                    var $lastRow = $('#detailBody .detail-row:last');
                    if (item.uom_pack) {
                        $lastRow.find('.inv-uom-pack-select').val(item.uom_pack);
                    }
                    if (item.uom_detail) {
                        $lastRow.find('.inv-uom-detail-select').val(item.uom_detail);
                    }
                });
...
$(document).on('change', '#order_no', function() {
    ...
    // Kosongkan detail items saat ganti order
    $('#detailBody').empty();
    rowCounter = 0;
    updateRowNumbers();
});
